r"""
    ...
    Modified from sphinx.ext.inheritance_diagram
    ...
    def generate_dot(
        ...
        *graph_attrs*, *node_attrs*, *edge_attrs* are dictionaries containing
        ...
        for name, fullname, bases, tooltip in sorted(self.class_info):
            # mermaidJS does not support '.' in class name
            mermaidName = name.replace(".", "_")
            # Write the node
            res.append("  class {!s}\n".format(mermaidName))
            if fullname in urls:
                res.append(
                    '  link {!s} "./{!s}" {!s}\n'.format(
                        mermaidName, urls[fullname], tooltip or '"{}"'.format(name)
                    )
                )
            ...
            for base_name in bases:
                res.append(
                    "  {!s} <|-- {!s}\n".format(
                        base_name.replace(".", "_"), mermaidName
                    )
                )
